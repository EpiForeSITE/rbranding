...
expect_true(file.exists(rprofile_path), 
            info = "install_rbranding_autoupdate should create .Rprofile")
...
config_path <- file.path(tmp_project, "rbranding_config.yml")
...
# Test 4: update_rbranding_autoupdate (should be idempotent on second call)
...
expect_equal(rprofile_content, rprofile_content2,
             info = "update_rbranding_autoupdate should be idempotent")  # Content should be same
...
expect_true(capture_result == "no_message",
            info = "install_rbranding_autoupdate should handle errors gracefully")
